...
def main(args):
    ...
    if torch.cuda.is_available() and args.gpu != '-1':
        ...
        torch.cuda.set_device(device=int(args.gpu))
    else:
    ...
    # Skip tensorboard logging for speed - just use log_every of 200
    ...
    for epoch in range(args.resume_epoch, args.nepochs):
        ...
        for ii, sample_batched in enumerate(trainloader):
            if 'trfe' in args.model_name or args.model_name == 'mtnet':
                # Check if we have both nodule and gland data
                if isinstance(sample_batched, (tuple, list)) and len(sample_batched) == 2:
                    nodules, glands = sample_batched
                    scale = nodules['scale'].to(device)
                    inputs_n, labels_n = nodules['image'].to(device), nodules['label'].to(device)
                    inputs_g, labels_g = glands['image'].to(device), glands['label'].to(device)
                    ...
                    if 'trfeplus' in args.model_name:
                        mse_loss_mini = (1 - epoch / args.nepochs) * soft_mse(pred_scales.to(device), scale.float())
                        loss += mse_loss_mini
                    ...
                    # Only nodule data (no gland), treat as regular segmentation
            ...
            if 'trfe' in args.model_name or args.model_name == 'mtnet':
                # Only log nodule/thyroid losses if we have both (gland data exists)
                if 'nodule_loss_mini' in locals() and 'thyroid_loss_mini' in locals():
                    epoch_nodule_losses.append(nodule_loss_mini.item())
                    epoch_thyroid_losses.append(thyroid_loss_mini.item())
                    nodule_loss = nodule_loss_mini.item()
                    thyroid_loss = thyroid_loss_mini.item()
        ...
        if not skip_tensorboard:
            ...
            if 'trfe' in args.model_name or args.model_name == 'mtnet':
                if len(epoch_nodule_losses) > 0:
                    writer.add_scalar('data/epoch_nodule_loss', sum(epoch_nodule_losses) / len(epoch_nodule_losses), nsamples)
                if len(epoch_thyroid_losses) > 0:
                    writer.add_scalar('data/epoch_thyroid_loss', sum(epoch_thyroid_losses) / len(epoch_thyroid_losses), nsamples)
        ...
        if args.use_test == 1:
            ...
            for ii, sample_batched in enumerate(testloader):
                # Handle both tuple (nodule, gland) and dict (nodule only) formats
                if isinstance(sample_batched, (tuple, list)):
                    sample_batched = sample_batched[0]  # Take nodule part if tuple
                inputs, labels = sample_batched['image'].to(device), sample_batched['label'].to(device)
                with torch.no_grad():
                    if 'trfe' in args.model_name or args.model_name == 'mtnet':
                        if 'trfeplus' in args.model_name:
