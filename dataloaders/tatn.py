...
class TATN(data.Dataset):
    def __init__(self, mode, transform=None, return_size=False, fold=0):
        ...
        if mode == 'train':
            ...
            print(f"Nodule data: {len(nodule_pathes)}, Gland data: {len(gland_pathes)}")
            
            if len(gland_pathes) == 0:
                print("WARNING: No gland data found, using nodule data only (TN3K mode)")
                nodule_imgs = nodule_pathes
            else:
    ...
    def __getitem__(self, item):
        if self.mode == 'train':
            ...
            # If gland data exists, return both
            if len(self.gland_imgs) > 0:
                ...
                gland['label_name'] = os.path.basename(label_path)
                gland['mark'] = 0
                return nodule, gland
            else:
                # No gland data, return only nodule
                return nodule
